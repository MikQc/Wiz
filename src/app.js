...
import { checkDueChannelLocks } from './services/channelLockService.js';
...
class TitanBot extends Client {
  ...
  setupCronJobs() {
    ...
    cron.schedule('* * * * *', runSafeTask('channel_lock_check', () => checkDueChannelLocks(this)));
  ...
  setupKeepAlive() {
    ...
    logger.info('[keepalive] Render detected, self-ping enabled every 5 minutes');
    cron.schedule('*/5 * * * *', () => {
      fetch(renderUrl)
        .then((res) => logger.info(`[keepalive] Pinged ${renderUrl} -> ${res.status}`))
        .catch((err) => logger.warn(`[keepalive] Ping failed for ${renderUrl}: ${err.message}`));
    });
  }
