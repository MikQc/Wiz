...
async function registerGlobalCommands(client, clientId, commands, totalSubcommands) {
    ...
    const existingRes = await client.rest.get(`/applications/${clientId}/commands`);
    const existingCount = Array.isArray(existingRes) ? existingRes.length : 0;
    ...
    if (botConfig.commands?.deleteCommands) {
        ...
        logger.info(`No commands found. Registering ${commandsToRegister.length} global commands...`);
        await client.rest.put(`/applications/${clientId}/commands`, { body: commandsToRegister });
        logger.info(`Successfully registered ${commandsToRegister.length} global commands`);
    } else {
        logger.info(`Skipped command registration: ${existingCount} commands already exist on Discord`);
    }
}
